...
import { ApproveAgentModal } from '@/shared/chakra/modals/ApproveAgentModal';
...
const PendingAgentPanel = () => {
  ...
  const columns = useMemo(
          ...
          cell: (info) => (
            <Menu>
              ...
              <MenuList>
                <MenuItem
                  onClick={() => {
                    setSelectedAgents([info.row.original]);
                    onOpen();
                  }}
                >
                  <Text as="span" variant="Body2Regular">
    ...
    [selectedAgents, onOpen]
  );

  return (
    <Flex flexDir="column" gap="1.5rem" w="100%" h="100%">
      <ApproveAgentModal isOpen={isOpen} onClose={onClose} agents={selectedAgents} />
      <Flex justifyContent="space-between">
        <InputGroup w="212px" size="sm">
          <InputLeftElement>
            <Icon as={MdSearch} w="12px" h="12px" color="primary.600" />

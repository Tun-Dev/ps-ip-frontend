...
type Props = {
  ...
  inputProps: UseFormRegisterReturn<`surveyForm.fields.${number}.name`>;
  index: number;
};

export function SurveyFormField({ field, inputProps, onDelete, onChange, index }: Props) {
          ...
          {fields.map((optionField, idx) => (
            <Flex key={optionField.id} gap="3" align="center">
              <Input
                ...
                borderColor="grey.300"
                w="fit-content"
                {...register(`surveyForm.fields.${index}.options.${idx}.label`)}
              />
